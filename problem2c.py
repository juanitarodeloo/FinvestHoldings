...
class PasswordManagement:
    ...
    def save_password_record(self, username, role, salt, hashed_password):
        ...
        with open('passwd.txt', 'a') as file:
            ...
            file.write(record) #TODO: handle error
    ...
    def get_role_num(self, username):
        ...
        return int(full_record.split(":")[1].strip()) #TODO: handle error

    def get_salt(self, username):
        """Returns the salt stored in the password file associated with the given username. """
        full_record = self.get_password_record(username)
        return full_record.split(":")[2].strip() #TODO: handle error

    def get_hashed_password(self, username):
        """Returns the hashed password stored in the password file associated with the given username. """
        full_record = self.get_password_record(username)
        return full_record.split(":")[3].strip() #TODO: handle error
